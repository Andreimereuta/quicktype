...
export declare class SwiftRenderer extends ConvenienceRenderer {
    ...
    private justTypesCase(justTypes, notJustTypes);
    ...
    private renderHeader(type, name);
    private renderTopLevelAlias(t, name);
    protected getProtocolsArray(_t: Type, isClass: boolean): string[];
    private getProtocolString(_t, isClass);
    ...
    private renderClassDefinition(c, className);
    protected initializableProperties(c: ClassType): SwiftProperty[];
    private emitNewEncoderDecoder();
    private emitConvenienceInitializersExtension(c, className);
    private renderEnumDefinition(e, enumName);
    private renderUnionDefinition(u, unionName);
    private emitTopLevelMapAndArrayConvenienceInitializerExtensions(t, name);
    private emitDecodingError(name);
    private emitSupportFunctions4;
    private emitConvenienceMutator(c, className);
